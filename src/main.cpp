...
#if defined(NDEBUG)
# error "Ezzycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa54708195135edfd55d44f766e048542aa6fb29c24006b1dbb71c621974ffd30");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ezzycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ezzycoin Signed Message:\n";
...
{
    // Ezzycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Ezzycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Ezzycoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Ezzycoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ezzycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ezzycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa54708195135edfd55d44f766e048542aa6fb29c24006b1dbb71c621974ffd30");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is ezzycoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1496382434;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1269297;
        ...
        assert(block.hashMerkleRoot == uint256("0xca400588600c7a9fe6036976572248722382779d9a8048455c783bb47b772df1"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Ezzycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EzzycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EzzycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EzzycoinMiner(CWallet *pwallet)
{
    printf("EzzycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ezzycoin-miner");
    ...
    try { loop {
        ...
        printf("Running EzzycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EzzycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EzzycoinMiner, pwallet));
}
